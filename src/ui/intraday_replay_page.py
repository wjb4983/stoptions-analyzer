...
class IntradayReplayPage(ttk.Frame):
    def __init__(self, parent: ttk.Frame, controller: StoptionsApp) -> None:
        ...
        ttk.Label(controls, text="Speed multiplier").grid(row=0, column=0, sticky="w")
        self.speed_var = tk.StringVar(value="1.0")
        ttk.Entry(controls, textvariable=self.speed_var, width=8).grid(row=0, column=1, padx=(8, 18), sticky="w")

        ttk.Button(controls, text="Load data", command=self.load_data).grid(row=0, column=2, padx=6)
        ttk.Button(controls, text="Play", command=self.start_replay).grid(row=0, column=3, padx=6)
        ttk.Button(controls, text="Stop", command=self.stop_replay).grid(row=0, column=4, padx=6)
        ttk.Button(controls, text="Back to Main Menu", command=lambda: controller.show_frame("MainMenu")).grid(
            row=0, column=5, padx=(24, 0)
        )
        ...
        self.bind("<Left>", lambda _evt: self.shift_day(-1))
        self.bind("<Right>", lambda _evt: self.shift_day(1))
    ...
    def load_data(self) -> None:
        ...
        if not ticker:
            messagebox.showinfo("Missing ticker", "Select a ticker first on the Select Stock page.")
            return
    ...
    def shift_day(self, offset: int) -> None:
        if not self.day_series:
            return
        self.stop_replay()
        self.day_index = max(0, min(len(self.day_series) - 1, self.day_index + offset))
        self.stage_index = 0
        self._draw_current_day()
