...
def _migrate_active_jobs(payload: object) -> dict[str, dict[str, object]]:
    ...
    for raw_job_id, raw_meta in payload.items():
        ...
        migrated[job_id] = {
            ...
            "server_run_dir": str(raw_meta.get("server_run_dir", "")).strip() or None,
            "summary_paths": dict(raw_meta.get("summary_paths", {})) if isinstance(raw_meta.get("summary_paths"), dict) else {},
            "summary_payload": dict(raw_meta.get("summary_payload", {})) if isinstance(raw_meta.get("summary_payload"), dict) else {},
            "schema_version": int(raw_meta.get("schema_version", 1) or 1),
